...
namespace Avalonia.Media
{
    ...
    public sealed class GlyphTypeface
    {
        ...
        public bool TryGetGlyphMetrics(ReadOnlySpan<ushort> glyphIndices, Span<GlyphMetrics> metrics)
        {
            ...
            // CFF / CFF2) in one pass. A font with no outline table keeps the buffer empty and falls
            // back to hmtx/vmtx bearings with a zero box.
            var hasInkTable = _glyfTable != null || _cffTable != null || _cff2Table != null;
            var boundsCount = hasInkTable ? glyphIndices.Length : 0;
            Span<GlyphBounds> bounds = boundsCount <= 256
                ? stackalloc GlyphBounds[boundsCount]
                : new GlyphBounds[boundsCount];

            var hasInk = hasInkTable && TryFillInkBounds(glyphIndices, bounds);
        ...
        private bool TryFillInkBounds(ReadOnlySpan<ushort> glyphIndices, Span<GlyphBounds> bounds)
        {
            ...
            if (_cffTable is not null || _cff2Table is not null)
            {
                for (int i = 0; i < glyphIndices.Length; i++)
                {
                    bounds[i] = TryGetCachedCffBounds(glyphIndices[i], out var box) ? box : default;
                }
        ...
        public IGeometryImpl? GetGlyphOutline(ushort glyphIndex)
        {
            ...
            if (Volatile.Read(ref cache.State[glyphIndex]) != 0)
            {
                return cache.Outlines[glyphIndex];
            }

            var outline = BuildGlyphOutline(glyphIndex);
            ...
            cache.Outlines[glyphIndex] = outline;
            Volatile.Write(ref cache.State[glyphIndex], 1);
        ...
        /// </summary>
        private IGeometryImpl? BuildGlyphOutline(ushort glyphIndex)
        {
            ...
            using (var ctx = geometry.Open())
            {
                ...
                else if (_cff2Table is not null)
                {
                    ...
                    built = _cff2Table.TryBuildGlyphGeometry((int)glyphIndex, Matrix.Identity, ctx, activeCoords);
                }
                else
                {
                    built = _cffTable!.TryBuildGlyphGeometry((int)glyphIndex, Matrix.Identity, ctx);
                }
