    ...
    internal static class PaintResolver
    {
        ...
        public static Paint ResolvePaint(Paint paint, in ColrContext context)
        {
            switch (paint)
            {
                ...
                // Format 10: Glyph
                case Glyph glyph:
                    return new Glyph(glyph.GlyphIndex, ResolvePaint(glyph.Paint, context));
        ...
        {
            var glyphIndex = colrGlyph.GlyphIndex;
            // Resolve inner paint
            var resolvedInner = ResolvePaint(colrGlyph.Inner, context);

            // Wrap in a clip box if present
            if (context.ColrTable.TryGetClipBox(glyphIndex, context.ActiveCoords, out var clipBox))
            {
