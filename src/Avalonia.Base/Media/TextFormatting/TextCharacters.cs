    ...
    public class TextCharacters : TextRun
    {
            ...
            ref TextRunProperties? previousProperties)
        {
            ...
            for (var capabilityTier = 0; capabilityTier < capabilityTiers; capabilityTier++)
            {
                ...
                // shares the primary's cmap. It still reclaims the whitespace between the words, which
                // needs no shaping - see TryGetShapeableLength.
        ...
        /// whitespace, which needs no shaping.
        /// </param>
            ...
            out int length)
        {
            ...
            while (enumerator.MoveNext(out var currentGrapheme))
            {
                ...
                // A fallback run ends where the default typeface regains coverage, whitespace included.
                // Whitespace used to be exempt so that a space wouldn't split a fallback run, but
                // practically every font maps U+0020: the exemption let a fallback run reach past the
                // text the default typeface couldn't render and shape the following space with the
                // fallback's own advance, which is a full em in most emoji fonts.
                // A default typeface that cannot shape this script still reclaims that whitespace: a
                // space carries no shaping, and leaving it with the fallback is what made the spaces
                // between two fallback words measure differently from the rest of the line.
                if (defaultGlyphTypeface != null
                    && (defaultCanShapeScript || currentCodepoint.IsWhiteSpace)
