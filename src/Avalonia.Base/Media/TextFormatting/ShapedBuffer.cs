...
namespace Avalonia.Media.TextFormatting
{
    public sealed class ShapedBuffer : IReadOnlyList<GlyphInfo>, IDisposable
    {
        ...
        private SplitResult<ShapedBuffer> SplitDescending(int textLength)
        {
            ...
            // because the whole cluster stays with "first". The text boundary has to follow it - the
            // ascending path snaps the same way. Cutting the text at the requested offset instead
            // leaves both halves holding glyphs for characters the other half claims, and every
            // later measure or split of either half then works off a cluster cache that no longer
            // describes its text.
