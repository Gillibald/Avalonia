...
/// <summary>
/// Maps rects between a host's local space and the space its dirty-rect
/// tracker stores in: texture pixels for a bitmap-cache host (mirroring the
/// cache collector proxy's offset/scale), identity for the render target's
/// tracker (device pixels).
/// </summary>
internal readonly struct DirtyRectSpaceMapping
{
    ...
    // A cache that has never drawn has a zero scale; nothing has been captured
    // into its tracker yet, so the mapping is simply unusable until the first
    // draw sets a real scale.
    public bool IsUsable => ScaleX != 0 && ScaleY != 0;

    public bool IsIdentity => ScaleX == 1 && ScaleY == 1 && DrawAtOffset == default;

    /// <summary>Host-local rect to tracker (texture) space. Matches the cache collector proxy's AddRect.</summary>
    ...
    /// <summary>Tracker (texture) space to host-local rect. Inverse of <see cref="HostToTracker"/>.</summary>
