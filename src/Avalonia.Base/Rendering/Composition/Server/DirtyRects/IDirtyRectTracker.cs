...
internal interface IDirtyRectTracker : IDirtyRectCollector
{
    ...
    /// <summary>
    /// Appends the currently accumulating working set (raw, in tracker space)
    /// to <paramref name="buffer"/> without finalizing, inflating or
    /// optimizing it. Safe to call mid-pass.
    /// </summary>
...
internal interface IDirtyRectCollector
{
    ...
    /// <summary>
    /// Resolves a side-effect-free view of the underlying tracker's working
    /// set, for walk-position backdrop classification. Collectors without a
    /// readable tracker (redirects, unions) return an empty view - consumers
    /// treat that as unknown provenance and classify conservatively.
    /// </summary>
