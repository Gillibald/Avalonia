...
namespace Avalonia.Documents
{
    ...
    internal static class ValidationHelper
    {
        ...
        internal static void VerifyPosition(ITextContainer container, ITextPointer position, string paramName)
        {
            ...
            {
                throw new ArgumentException("NotInAssociatedTree", paramName);
            }
        }
        ...
        internal static void VerifyPositionPair(ITextPointer startPosition, ITextPointer endPosition)
        {
            ...
            {
                throw new ArgumentException($"InDifferentTextContainers {startPosition} {endPosition}");
            }
            if (startPosition.CompareTo(endPosition) > 0)
            {
                throw new ArgumentException($"BadTextPositionOrder {startPosition} {endPosition}");
            }
        }
        ...
        internal static void ValidateChild(TextPointer position, object child, string paramName)
        {
            ...
            {
                throw new ArgumentException($"TextSchema_ChildTypeIsInvalid {position.Parent.GetType().Name}, {child.GetType().Name}");
            }
            ...
            if (child is TextElement)
            {
                ...
                {
                    throw new ArgumentException($"TextSchema_TheChildElementBelongsToAnotherTreeAlready {child.GetType().Name}");
                }
            }
            else
            {
