...
namespace Avalonia.Documents
{
    ...
    internal static class TextPointerBase
    {
        ...
        private static bool IsAtPotentialRunPosition(ITextPointer backwardPosition, ITextPointer forwardPosition)
        {
            ...
            {
                var forwardType = forwardPosition.GetElementType(LogicalDirection.Forward);
                var backwardType = backwardPosition.GetElementType(LogicalDirection.Backward);
                if (forwardType != null && backwardType != null)
                {
                    var forwardContext = forwardPosition.GetPointerContext(LogicalDirection.Forward);
                    var backwardContext = backwardPosition.GetPointerContext(LogicalDirection.Backward);
        ...
        {
            var parentType = position.ParentType;
            var backwardContext = position.GetPointerContext(LogicalDirection.Backward);
            var forwardContext = position.GetPointerContext(LogicalDirection.Forward);
        ...
        {
            var navigator = position.CreatePointer();
            var parentType = navigator.ParentType;
        ...
        internal static ITextPointer GetFollowingNonMergeableInlineContentStart(ITextPointer position)
        {
            ...
            while (true)
            {
                var category = GetBorderingElementCategory(navigator, LogicalDirection.Forward);
        ...
        internal static bool IsAtWordBoundary(ITextPointer thisPosition, LogicalDirection insideWordDirection)
        {
            ...
            var navigator = thisPosition.CreatePointer();
        ...
        private static ITextPointer RestrictWithinBlock(ITextPointer position, ITextPointer limit, LogicalDirection direction)
        {
            ...
            while (direction == LogicalDirection.Backward ? position.CompareTo(limit) > 0 : position.CompareTo(limit) < 0)
            {
                ...
                {
                    var elementType = position.GetElementType(direction);
        ...
        internal static bool IsAtLineWrappingPosition(ITextPointer position, ITextView textView)
        {
            ...
            var lineSegment = textView.GetLineRange(position);
        ...
        internal static ListItem GetListItem(TextPointer pointer)
        {
            ...
            var paragraphOrBlockUIContainer = pointer.ParagraphOrBlockUIContainer;
        ...
        internal static ListItem GetImmediateListItem(TextPointer position)
        {
            ...
            var paragraphOrBlockUIContainer = position.ParagraphOrBlockUIContainer;
        ...
        internal static int MoveToLineBoundary(ITextPointer thisPointer, ITextView textView, int count, bool respectNonMeargeableInlineStart)
        {
            ...
            {
                var lineRange = textView.GetLineRange(position);
        ...
        {
            var textView = thisPointer.TextContainer.TextView;
            ...
            {
                var insertionPosition = thisPointer.GetInsertionPosition(direction);
            ...
            var rect = textView.GetRectangleFromTextPosition(thisPointer.CreatePointer(direction));
        ...
        internal static bool MoveToNextInsertionPosition(ITextPointer thisNavigator, LogicalDirection direction)
        {
            ...
            var initialPosition = thisNavigator.CreatePointer();
            ...
            if (direction == LogicalDirection.Backward)
            {
                ...
                var context = thisNavigator.GetPointerContext(direction);
        ...
        internal static bool MoveToNextWordBoundary(ITextPointer thisNavigator, LogicalDirection movingDirection)
        {
            ...
            Invariant.Assert(!thisNavigator.IsFrozen, "Can't reposition a frozen pointer!");
            var startPosition = thisNavigator.CreatePointer();
        ...
        private static bool IsInsideCompoundSequence(ITextPointer position)
        {
                ...
                position.GetTextInRun(LogicalDirection.Forward, neighborhood, 1, 1) == 1)
            {
                ...
                // combining mark.
                //
                var category1 = char.GetUnicodeCategory(neighborhood[1]);
                ...
                {
                    var category0 = char.GetUnicodeCategory(neighborhood[0]);
        ...
        {
            var elementType = GetBorderingElementCategory(position, direction);
        ...
        {
            var context = direction == LogicalDirection.Forward ? TextPointerContext.ElementEnd : TextPointerContext.ElementStart;
        ...
        private static bool IsNextToRichBreak(ITextPointer thisPosition, LogicalDirection direction, Type lineBreakType)
        {
            ...
            while (true)
            {
                var neighbor = thisPosition.GetElementType(direction);
