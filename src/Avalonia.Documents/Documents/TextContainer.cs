...
namespace Avalonia.Documents
{
    ...
    internal class TextContainer : ITextContainer
    {
        ...
        {
#if DEBUG
            return "TextContainer Id=" + _debugId + " SymbolCount=" + SymbolCount;
#else
        ...
        internal void EnableUndo(IAvaloniaObject uiScope)
        {
        ...
        internal void DisableUndo(IAvaloniaObject uiScope)
        {
        ...
        internal StaticTextPointer CreateStaticPointerAtOffset(int offset)
        {
            ...
            nodeOffset = offset + 1 - node.GetSymbolOffset(Generation);
        ...
        internal int GetInternalOffset(StaticTextPointer position)
        {
            ...
            {
                offset = node.GetSymbolOffset(Generation) + nodeOffset;
            }
            else
            {
        ...
        int ITextContainer.GetTextInRun(StaticTextPointer position, LogicalDirection direction, char[] textBuffer, int startIndex, int count)
        {
            ...
            return textNode == null ? 0 : TextPointer.GetTextInRun(this, textNode.GetSymbolOffset(Generation), textNode, nodeOffset, direction, textBuffer, startIndex, count);
        }
        ...
        StaticTextPointer ITextContainer.GetNextContextPosition(StaticTextPointer position, LogicalDirection direction)
        {
            ...
            {
                edge = node.GetEdgeFromOffset(nodeOffset, LogicalDirection.Forward);
                moved = TextPointer.GetNextNodeAndEdge(node, edge, PlainTextOnly, out node, out edge);
            }
            else
            {
                edge = node.GetEdgeFromOffset(nodeOffset, LogicalDirection.Backward);
                moved = TextPointer.GetPreviousNodeAndEdge(node, edge, PlainTextOnly, out node, out edge);
            }
        ...
        internal void InsertTextInternal(TextPointer position, object text)
        {
            ...
            originalPosition = HasListeners ? new TextPointer(position, LogicalDirection.Backward) : null;
            ...
            // Insert the raw text.
            symbolOffset = textNode.GetSymbolOffset(Generation);
        ...
        internal void InsertElementInternal(TextPointer startPosition, TextPointer endPosition, TextElement element)
        {
            ...
            if (newElementNode)
            {
                ...
                element.TextElementNode = elementNode;
            }
        ...
        internal void InsertEmbeddedObjectInternal(TextPointer position, IAvaloniaObject embeddedObject)
        {
            ...
            symbolOffset = objectNode.GetSymbolOffset(Generation);
        ...
        internal bool IsAtCaretUnitBoundary(TextPointer position)
        {
            ...
            if (_rootNode.CaretUnitBoundaryCacheOffset != position.GetSymbolOffset())
            {
                ...
                if (!_rootNode.CaretUnitBoundaryCache && position.LogicalDirection == LogicalDirection.Backward)
                {
                    ...
                    var positionForwardGravity = position.GetPositionAtOffset(0, LogicalDirection.Forward);
        ...
        internal bool HasListeners
        {
            get
            {
                return ChangingHandler != null || ChangeHandler != null || ChangedHandler != null;
            }
        }
        ...
        private int DeleteContentFromSiblingTree(SplayTreeNode containingNode, TextPointer startPosition, TextPointer endPosition, bool newFirstIMEVisibleNode, out int charCount)
        {
            ...
            rootNode = SplayTreeNode.Join(leftSubTree, rightSubTree);
        ...
        private int CutTopLevelLogicalNodes(TextTreeNode containingNode, TextPointer startPosition, TextPointer endPosition, out int charCount)
        {
            ...
            while (node != stopNode)
            {
                ...
                if (elementNode != null)
                {
                    ...
                    elementText = TextTreeText.CutText(_rootNode.RootTextBlock, elementNode.GetSymbolOffset(Generation), elementNode.SymbolCount);
        ...
        private void AssertTreeAndTextSize()
        {
            if (Invariant.Strict)
            {
                ...
                if (_rootNode.RootTextBlock != null)
                {
                    ...
                    Invariant.Assert(count == InternalSymbolCount, "TextContainer.SymbolCount does not match TextTreeText size!");
                }
            }
        }
        ...
        //    get
        //    {
        //        return Parent != null ? Parent.Dispatcher : null;
        //    }
        //}
...
#if DEBUG
        private int _debugId = SplayTreeNode.GetDebugId();
#endif // DEBUG
