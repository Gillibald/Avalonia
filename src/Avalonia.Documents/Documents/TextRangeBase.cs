...
namespace Avalonia.Documents
{
    ...
    internal static class TextRangeBase
    {
        ...
        internal static bool Contains(ITextRange thisRange, ITextPointer textPointer)
        {
            ...
            {
                throw new ArgumentException($"SRID.NotInAssociatedTree {textPointer}");
            }
        ...
        internal static void SelectWord(ITextRange thisRange, ITextPointer position)
        {
            ...
            var normalizedPosition = position.CreatePointer();
            normalizedPosition.MoveToInsertionPosition(LogicalDirection.Backward);

            var wordRange = TextPointerBase.GetWordRange(normalizedPosition);
        ...
        {
            var autoWordRange = TextSegment.Null;
                ...
                !TextPointerBase.IsAtWordBoundary(thisRange.Start, LogicalDirection.Backward))
            {
                ...
                string autoWord = GetTextInternal(autoWordRange.Start, autoWordRange.End).TrimEnd(' ');

                string textFromWordStart = GetTextInternal(autoWordRange.Start, thisRange.Start);
        ...
        internal static void ApplyFinalTypingHeuristics(ITextRange thisRange, bool overType)
        {
                ...
                !TextPointerBase.IsNextToAnyBreak(thisRange.End, LogicalDirection.Forward))
            {
                ...
                var nextPosition = thisRange.End.CreatePointer();
            ...
            {
                var newEnd = thisRange.End.GetNextInsertionPosition(LogicalDirection.Backward);
        ...
        {
            var nextContext = navigator.GetPointerContext(direction);
        ...
        private static object GetCharacterPropertyValue(ITextRange thisRange, AvaloniaProperty formattingProperty)
        {
            ...
            for (int i = 0; i < thisRange._TextSegments.Count; i++)
            {
                var textSegment = thisRange._TextSegments[i];

                var position = textSegment.Start.CreatePointer();
                bool moved = true;
        ...
        private static object GetCharacterValueFromPosition(ITextPointer pointer, AvaloniaProperty formattingProperty)
        {
            ...
            else
            {
                ...
                {
                    AvaloniaObject element = ((TextPointer)pointer).Parent as TextElement;
        ...
        {
            var startNavigator = thisRange.Start.CreatePointer();
            var endNavigator = thisRange.End.CreatePointer();
        ...
        internal static void EndChange(ITextRange thisRange, bool disableScroll, bool skipEvents)
        {
            ...
            finally
            {
                ...
                changeBlockUndoRecord = thisRange._ChangeBlockUndoRecord;
        ...
        internal static string GetTextInternal(ITextPointer startPosition, ITextPointer endPosition, ref char[] charArray)
        {
            ...
            var navigator = startPosition.CreatePointer();
            ...
            while (navigator.CompareTo(endPosition) < 0)
            {
                ...
                var symbolType = navigator.GetPointerContext(LogicalDirection.Forward);
                switch (symbolType)
                {
        ...
        internal static bool GetIsEmpty(ITextRange thisRange)
        {
            ...
            Invariant.Assert(
                ...
                thisRange._TextSegments[0].Start == (object)thisRange._TextSegments[0].End)
                ==
        ...
        internal static string GetText(ITextRange thisRange)
        {
            ...
            if (!thisRange.IsTableCellRange)
            {
                ...
                return GetTextInternal(start, thisRange.End);
            }
            else
            {
                string text;
            ...
            try
            {
                ...
                if (!thisRange.IsEmpty)
                {
                        ...
                        textData.Length > 0)
                    {
                        ...
                        var textContainer = ((TextPointer)thisRange.Start).TextContainer;
                ...
                {
                    var insertPosition = explicitInsertPosition == null ? thisRange.Start : explicitInsertPosition;
                    ...
                    var newStart = insertPosition.GetFrozenPointer(LogicalDirection.Backward);
                    var newEnd = insertPosition.CreatePointer(LogicalDirection.Forward);
                    ...
                    // Select the range
                    SelectPrivate(thisRange, newStart, newEnd, /*includeCellAtMovingPosition:*/false, /*markRangeChanged*/true);
                }
            }
            finally
            {
        ...
        {
            var start = range.Start.CreatePointer();
            ...
            {
                var end = range.End.CreatePointer();
        ...
        {
            var textContainer = thisRange.Start.TextContainer;
        ...
        private static void NormalizeRange(ITextRange thisRange)
        {
            ...
            var start = thisRange._TextSegments[0].Start;
            var end = thisRange._TextSegments[thisRange._TextSegments.Count - 1].End;
        ...
        private static void SelectPrivate(ITextRange thisRange, ITextPointer position1, ITextPointer position2, bool includeCellAtMovingPosition, bool markRangeChanged)
        {
            ...
            else
            {
                ...
                var newStart = position1;
                var newEnd = position2;
                ...
                CreateNormalizedTextSegment(thisRange, newStart, newEnd);
                ...
                if (position1 is TextPointer)
                {
                    var finalStart = thisRange._TextSegments[0].Start;
                    var finalEnd = thisRange._TextSegments[thisRange._TextSegments.Count - 1].End;
