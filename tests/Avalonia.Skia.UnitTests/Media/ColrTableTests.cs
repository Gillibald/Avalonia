...
namespace Avalonia.Skia.UnitTests.Media
{
    public class ColrTableTests
    {
        ...
        private class DetailedDiagnosticPainter : IColorPainter
        {
            ...
            public void Glyph(ushort glyphIndex)
            {
                var geometry = _glyphTypeface.GetGlyphOutline(glyphIndex);
                var bounds = geometry?.Bounds ?? default;
                var transform = _transforms.Peek();
                var transformedBounds = bounds.TransformToAABB(transform);

                Operations.Add($"[{_operationIndex++}] Glyph: GlyphID={glyphIndex}");
            ...
            public void ColrGlyph(ushort glyphIndex)
            {
                Operations.Add($"[{_operationIndex++}] ColrGlyph: GlyphID={glyphIndex}");
            }
        }
